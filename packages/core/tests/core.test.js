...
import { X402_HEADERS, SUPPORTED_TOKENS, getDefaultTokens, } from "../src/constants";
// ─── Fixtures ─────────────────────────────────────────────────────────────────
...
describe("validatePaymentRequirements", () => {
    ...
    it("rejects invalid scheme", () => {
        const result = validatePaymentRequirements(makeRequirements({ scheme: "unknown" }));
        expect(result.valid).toBe(false);
        expect(result.errors.some((e) => e.includes("scheme"))).toBe(true);
    });
    it("rejects invalid network", () => {
        const result = validatePaymentRequirements(makeRequirements({ network: "ethereum:1" }));
        expect(result.valid).toBe(false);
        expect(result.errors.some((e) => e.includes("network"))).toBe(true);
    });
    it("rejects amount below minimum", () => {
        const result = validatePaymentRequirements(makeRequirements({ maxAmountRequired: "1" }));
        expect(result.valid).toBe(false);
    ...
    it("requires streaming rate for streaming scheme", () => {
        const result = validatePaymentRequirements(makeRequirements({ scheme: "streaming" }));
        expect(result.valid).toBe(false);
...
describe("validatePaymentPayload", () => {
    ...
    it("rejects unsupported token", () => {
        const errors = validatePaymentPayload(makePayload({ token: "DOGE" }));
    ...
    it("rejects expired payment (> 10 min old)", () => {
        const errors = validatePaymentPayload(makePayload({ timestamp: Date.now() - 11 * 60 * 1000 }));
...
describe("bitflow", () => {
    ...
    it("throws for unsupported pair", async () => {
        await expect(getBitflowQuote("sBTC", "sBTC", "100")).rejects.toThrow("not supported");
    });
});
